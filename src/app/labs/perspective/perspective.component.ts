...
export class PerspectiveComponent {
  ...
  calcPerspective(x: number, y: number) {
    ...
    TweenMax.to(this.transform, 0.2, {
      tx: tiltX,
      ty: tiltY,
      d: deg,
      onUpdate: this.updateBoxTransform,
      onUpdateScope: this,
      ease: Cubic.easeOut
    });
  }
  ...
  resetBox() {
    TweenMax.to(this.transform, 0.4, {
      tx: 0,
      ty: 0,
      d: 0,
      onUpdate: this.updateBoxTransform,
